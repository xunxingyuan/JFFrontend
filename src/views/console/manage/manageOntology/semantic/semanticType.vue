<template>
  <div class="semanticType">
    ...
    <div class="content scrollbar">
      <section>
        <p class="title">
          <span v-if="node === 'owl:Thing'"></span>
          <span v-else>{{selectNode.subject.browserText}}</span>的具体描述
        </p>
        <div class="inputItem">
          <p>名称</p>
        ...
        <div class="inputItem">
          <p class="flex" style="align-items: center">属性
            <span class="smallBtn btn_normal" @click="addPropertyItem()">添加</span>
          </p>
  ...
  watch: {
    classMsg: function() {
      if (this.classMsg) {
        ...
        if (
          this.classMsg.hasOwnProperty("parents") &&
          this.classMsg.parents.length > 0
        ) {
