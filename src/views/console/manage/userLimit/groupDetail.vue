<template>
  <div class="groupDetail container">
    <div class="top">
      <p class="title"><i class="fas fa-arrow-left" @click="back" ></i> 用户信息</p>
    </div>
    <div class="bottom">
      <div class="content">
        <div class="flex">
  ...
  export default {
    ...
    methods:{
      ...
      deleteGroupUser: function (item) {
        this.$confirm('此操作将从用户组删除该用户（不会删除该用户）, 是否继续?', '提示', {
          confirmButtonText: '确定',
          cancelButtonText: '取消',
          type: 'warning'
        }).then(()=>{
          this.$api.user.userGroup.delGroupUser({
            userId: item.userId,
            groupId: this.$route.query.groupId
          }).then((res)=>{
            if(res.status === 200){
      ...
      _initData: function () {
        this.$api.user.userGroup.getGroupUserList({
          groupId: this.$route.query.groupId
        }).then((res)=>{
          if(res.status === 200){
