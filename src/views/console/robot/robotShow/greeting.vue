...
//        console.log(val)
      ...
      getCategoryQa: function (id) {
        ...
        if(id!=='weifenlei'){
          ...
          this.$api.robotAnalysis.greeting.searchQaNew(this.filterData).then((res)=>{
            if(res.code === 'ok'){
      ...
      getQaAIML: function (item) {
        this.selectQa = item
        this.$api.robotAnalysis.greeting.getQaAiml({
          qaId: item.qaId
        }).then((res)=>{
          if(res.code === 'ok'){
      ...
      deleteAiml: function (item) {
        ...
        this.$confirm('此操作将永久删除该AIML, 是否继续?', '删除', {
          ...
          this.$api.robotAnalysis.greeting.deleteAiml({
      ...
      //生成aiml
      submitAiml: function () {
        this.$api.robotAnalysis.greeting.updateQaAiml({
          isAll: true,
          status: 1,
          checkStatus: 4,
          qaIds: []
...
//        console.log(val)
      ...
      deleteQa: function (item) {
        this.$confirm('此操作将永久删除该寒暄, 是否继续?', '删除', {
          ...
          this.$api.robotAnalysis.greeting.deleteQa({
            qaId: item.qaId
          }).then((res)=>{
            if(res.code === 'ok'){
              this.$message({
      ...
      publishAIML: function (item) {
        this.$api.robotAnalysis.greeting.makeAIML({
          qaId: item.qaId
        }).then((res)=>{
          if(res.code === 'ok'){
            this.$message({
      ...
      delCategory: function () {
        if(this.selectNode&&this.selectNode.categoryId!=='weifenlei'&&this.selectNode.categoryId!=='0'){
          this.$confirm('此操作将永久删除该分类及其子分类, 是否继续?', '删除', {
            ...
            this.$api.robotAnalysis.greeting.deleteCategory({
              categoryId: this.selectNode.categoryId
            }).then((res)=>{
              if(res.code === 'ok'){
                this.$message({
...
//                this.treeData = arr
      ...
      showUpload: function () {
        if(this.selectNode&&this.selectNode.categoryId!=='0'&&this.selectNode.categoryId!=='weifenlei'){
          ...
          this.uploadUrl = '/api/'+ this.uploadData.botId + '/greeting/importExcel'
        }else{
          this.$message({
            type: 'info',
