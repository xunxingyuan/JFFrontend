<template>
  <div class="greeting container">
    ...
    <div class="bottom">
      <div class="content">
        ...
        <div v-if="activeMode=== 1" class="innerRight scrollbar">
          <div class="rightTop">
            <div class="add">
              <el-button type="primary" size="small" @click="createQa">+ 新建寒暄</el-button>
              <el-button size="small" type="success" @click="createGlobalAiml">同步寒暄</el-button>
              <el-button size="small" @click="showUpload">导入寒暄</el-button>
            </div>
          ...
          <div class="rightBottom">
            <div class="rightContent">
                ...
                <tr v-for="(item,index) in questionList" :key="index">
                  ...
                  <td>
                    <el-checkbox-group v-model="checkList">
                      <el-checkbox
                        :label="totalCount- (filterData.pageNum-1)*10 -index"
                        :key="index+1"
                      ></el-checkbox>
                    </el-checkbox-group>
                    <!--{{index+1}}-->
                  ...
                  <td :title="item.question">
                    <div
                      class="text"
                      style="color: #2b86f6;cursor:point;"
                      @click="editQa(item)"
                    >{{item.question}}</div>
                  </td>
              ...
              <p v-if="questionList.length===0" class="tips">暂无相关寒暄</p>
              <div class="pageBox">
                <el-pagination
                  background
    ...
    <qa-edit
      ...
      :editData="choseEditData"
      @submit="submitData"
      @submitAndAiml="submitAndAiml"
    ></qa-edit>
  ...
  methods: {
    ...
    changeUserLimit: function() {
      ...
      if (!this.userAuth.admin) {
        ...
        authData.forEach(e => {
          if (e.conversationType === "greeting") {
    ...
    _initData: function() {
      ...
      if (role === "admin") {
        this.$api.robotAnalysis.greeting
          .getCategory({
            categoryPId: 0
          })
          .then(res => {
        ...
        this.$api.user.menu
          ...
          .then(res => {
            if (res.status === 200) {
              this.treeData = res.data.greeting;
            }
          });
      }
    },
    ...
    createGlobalAiml: function() {
      this.$confirm("此操作将所有已保存的寒暄生成AIML, 是否继续?", "提示", {
        confirmButtonText: "确定",
        cancelButtonText: "取消",
        type: "warning"
      }).then(() => {
    ...
    searchQaNew: function() {
      ...
      this.$api.robotAnalysis.greeting.searchQaNew(filterData).then(res => {
        if (res.code === "ok") {
          this.questionList = res.result;
          this.totalCount = res.totalSize;
        } else {
    ...
    submitData: function(val) {
      ...
      this.$api.robotAnalysis.greeting.createQa(val).then(res => {
        if (res.status === 200) {
          this.$message({
            type: "success",
            message: "保存成功",
    ...
    submitAndAiml: function(val) {
      ...
      this.$api.robotAnalysis.greeting.createQaAiml(val).then(res => {
        if (res.status === 200) {
          this.$message({
            type: "success",
            message: "保存成功",
    ...
    editQa: function(item) {
      this.selectQa = item;
      if (this.sceneLimit.edit || this.sceneLimit.view) {
        this.$api.robotAnalysis.greeting
          .getQaDetail({
            qaId: item.qaId
          })
    ...
    deleteQa: function(item) {
      this.$confirm("此操作将永久删除该寒暄, 是否继续?", "删除", {
        confirmButtonText: "确定",
        cancelButtonText: "取消",
        type: "warning"
      }).then(() => {
    ...
    addCategory: function() {
      ...
      this.$prompt(
        "是否在分类：(" + this.selectNode.categoryName + ")下创建分类？",
        "分类",
        {
          confirmButtonText: "确定",
          cancelButtonText: "取消"
        ...
        this.$api.robotAnalysis.greeting.createCategory(data).then(res => {
          if (res.code === "ok") {
            this.$message({
              type: "success",
              message: "创建分类成功",
    ...
    editCategory: function() {
      if (
        this.selectNode &&
        this.selectNode.categoryId !== "weifenlei" &&
        this.selectNode.categoryId !== "0"
      ) {
        this.$prompt("请输入分类名称", "更新分类", {
          ...
          this.$api.robotAnalysis.greeting.createCategory(data).then(res => {
            if (res.code === "ok") {
              ...
              let arr = this.treeData;
              treeTools.treeCtrl.updateToTree(
                arr,
          ...
          this.$api.robotAnalysis.greeting
            ...
            .then(res => {
              if (res.code === "ok") {
                ...
                let arr = this.treeData;
                treeTools.treeCtrl.deleteFromTree(
                  arr,
                  this.selectNode.categoryId,
                  "children",
                  "categoryId"
    ...
    showUpload: function() {
      if (
        ...
        this.uploadUrl =
          "/api/" + this.uploadData.botId + "/greeting/importExcel";
      } else {
        this.$message({
          type: "info",
    ...
    choseOrUnChose: function() {
      ...
      if (this.checkList.length === len) {
        ...
        this.questionList.forEach((e, index) => {
          let result =
            this.totalCount - (this.filterData.pageNum - 1) * 10 - index;
